...
`combine` function is not supplied, `combine` defaults to addition except for
Booleans for which `combine` defaults to disjunction. All elements of `I` must satisfy
